...
def configLoader(param : str = 'default') -> bool:
    ...
    configPath = f'{getcwd()}/autofish2.config'
    ...
    def resuply(gc: any = None):
        for _query in _queries:
            try:
                ...
                if _query == _queries[-1]:
                    sleep(USER_COOLDOWN)
                else: sleep(2)
            except Exception as e:
                log(f'Failed to send query -> {e}', 'e', stack())
...
def log(message : any, type: str = 'normal', caller: Callable[[], Any] = None) -> None:
    global log_message
    msg = sub(r'\n\r', '', str(message))
    if type == 'normal':
        log_message = f'[*] {msg}'
...
def debug(event: any, caller: stack = None) -> None:
    if DEBUG: 
        print(f'[DEBUG]\nStack -> {caller=} -> {event}\n\n')

def pause(func: Callable[[], Any] = None, fparam: any = None, resumefunc: object = None, rparam: any = None) -> None:
    global fish_paused
    if func:
        #Pause between some function
        fish_paused = True
        log(f'Autofish paused. Calling function: {func.__name__}()', caller=stack())
        func(fparam)
        log(f'Autofish resumed.', caller=stack())
        fish_paused = False
    else:
        if fish_paused:
            #Resume
...
def main():
    ...
    atp = 0
    while True:
        cooldown_lifetime = round(uniform((USER_COOLDOWN - MARGIN / 3), (USER_COOLDOWN + MARGIN)), 3)
        ...
        if captcha.detected and atp <= 3:
            try:
                ...
                log(f'Manual captcha is required. -> {e}', 'e', caller=stack())
        
        while True:
            try: response = session.recieveEvent()
            except websocket.WebSocketConnectionClosedException as e:
                log(f'Connection lost -> {e}. Attempting to reconnect', 'e', caller=stack())
                session.connect()
            except Exception as e:
                exit(f'[E] Something went wrong -> {e}')
                        ...
                        if captcha.detect(event):
                            ...
                            if message.title:
                                ...
                                if message.title.find('Statistics for') > -1:
                                    ...
                                    log(f'Stats information updated.', caller=stack())
                                    pass
                                elif message.title.find('Inventory of') > -1:
                                    profile.rebuildLists(event, 'inv')
                                    log('Profile information updated.', caller=stack())
                                    pass
                                
                                #Normal messages
                                elif message.title == 'You caught:':
                                    message.buildList()
                                    break
                                else:
                                    #Unhandled
                                    log(message.title, caller=stack())
                                    pass
                            else:
                                #Untitled
                                log(message.items_list[0], caller=stack())
                                if message.items_list[0].find('You must wait') > -1:
                                    log(f'If automatic, this cooldown is intentional to ensure noise.', 'notice')
                                    session.sendQuery('%f')
                                pass
